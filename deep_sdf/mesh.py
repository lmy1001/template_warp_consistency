...
def create_mesh(
    decoder, on_data, code_input, filename, N=256, max_batch=(32 ** 3 * 4), offset=None, scale=None, volume_size=2.0
):
    start = time.time()
    ...
    while head < num_samples:
        ...
        samples[head : min(head + max_batch, num_samples), 3] = (
            deep_sdf.utils.decode_sdf(decoder, code_input, sample_subset.unsqueeze(0), on_data)
            .squeeze(1)
            .detach()
            .cpu()
        )
...
def create_mesh_octree(
        decoder, on_data, code_input, filename, N=256, max_batch=32 ** 3, offset=None, scale=None, clamp_func=None,
        volume_size=2.0):
    start = time.time()
    ply_filename = filename
    ...
    while reso > 0:
        ...
        while head < samples_.shape[0]:
            ...
            s = (deep_sdf.utils.decode_sdf(
                    decoder, code_input, samples_[query_idx, :3].unsqueeze(0), on_data).view([-1]).detach()
            )
